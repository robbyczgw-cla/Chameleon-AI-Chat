...
ALTER COLUMN selected_model SET DEFAULT 'x-ai/grok-4.1-fast:free';

-- Update the default value for selected_models array column
ALTER TABLE public.user_settings
ALTER COLUMN selected_models SET DEFAULT ARRAY['x-ai/grok-4.1-fast:free'];
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    ALTER COLUMN model SET DEFAULT 'x-ai/grok-4.1-fast:free';
  END IF;
END $$;
...
SET
  ...
  selected_models = array_prepend('x-ai/grok-4.1-fast:free',
    array_remove(selected_models, 'x-ai/grok-4-fast:free')),
  updated_at = NOW()
WHERE selected_model = 'x-ai/grok-4-fast:free';
...
BEGIN
  ...
  RAISE NOTICE 'Updated % user(s) to use x-ai/grok-4.1-fast:free as default model', affected_rows;
END $$;
